...
export default function ClientsPage() {
  ...
  useEffect(() => {
    ...
    if (mq.addEventListener) mq.addEventListener("change", updatePageSize);
    else mq.addListener(updatePageSize);

    return () => {
      if (mq.removeEventListener) mq.removeEventListener("change", updatePageSize);
      else mq.removeListener(updatePageSize);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    async function fetchClients() {
      ...
      else console.error("Error loading clients:", error);
    }
  ...
  //helpers for opening/closing modal
  ...
  const processed = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      const stringCompare = (x, y) =>
        String(x || "").localeCompare(String(y || ""), undefined, { sensitivity: "base" }) * dir;
      ...
      if (sortField === "service_city") return stringCompare(a.service_city, b.service_city);
        ...
        <div className={styles.headerRow}>
          ...
          {hasClients && (
            <div className={styles.headerButtons}>
              <button className={styles.uaddButton} onClick={() => setShowUpload(true)}>
                Import Clients
              </button>

              <button className={styles.addButton} onClick={openCreateClient}>
                Add Client
              </button>
            </div>
          )}
        ...
        {!hasClients ? (
          <div className={styles.emptyState}>
            ...
            <div className={styles.headerButtons}>
              ...
              <button className={styles.addFirstButton} onClick={openCreateClient}>
          ...
          <>
            ...
            {processed.totalItems === 0 ? (
              <div className={styles.noResultsBox}>
                <p className={styles.noResultsText}>No clients match your search.</p>
              </div>
            ) : (
              <>
                ...
                <div className={styles.paginationRow}>
                  ...
                  <div className={styles.pageControls}>
                    ...
                    {Array.from({ length: processed.totalPages }, (_, idx) => idx + 1).map((page) => (
                      <button
                        key={page}
